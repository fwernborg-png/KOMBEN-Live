...
export function buildFinalSignalNotificationKey(
  args: {
    raceDate: string;
    trackId: number;
    raceNumber: number;
...
function buildTrialNotification(
  args: {
    trackName: string;
    raceNumber: number;
  ...
  if (
    ...
    const sameHorse =
      winnerCandidate.runnerNumber ===
      placeCandidate.runnerNumber;

    return {
      title:
        `TVÅ SPEL – ` +
        `${trackName} lopp ${raceNumber}`,

      body:
        `VINNARE ${WIN_PLACE_RULE_CONFIG_V1.defaultWinStakeSEK} kr: ` +
        `${horseLabel(winnerCandidate)} · ` +
        `startodds ${formatOdds(winnerCandidate.startOdds)} · ` +
  ...
  const candidate =
    placeCandidate as SmoothestCandidate;
  ...
  return {
    ...
    body:
      ...
      `Jämnaste hästen · ` +
      `låsodds ${formatOdds(candidate.currentWinOdds)} · ` +
      `styrka ${candidate.strength}/6.`,
  };
}

export function buildFinalSignalNotification(
  args: {
    raceDate: string;
    trackId: number;
    trackName: string;
  ...
  const sameHorse =
    winPlaceCandidate !== null &&
    placeCandidate !== null &&
    winPlaceCandidate.runnerNumber ===
      placeCandidate.runnerNumber;
