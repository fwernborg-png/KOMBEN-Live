-- RESEARCH HISTORY ALL RUNNERS V4
--
...
drop function if exists public.research_history_rows_v4(
  date,
  date,
  text,
  text,
...
create function public.research_history_rows_v4(
...
begin
  ...
  where (
    v_selection = 'ALL_RUNNERS'
    or candidate.candidate_order = 1
  )
...
revoke all
  on function public.research_history_rows_v4(
    date,
    date,
    text,
    text,
...
comment on function public.research_history_rows_v4(
  date,
  date,
  text,
  text,
...
is
  'Strategihäst eller alla startande hästar med kombinerbara filter för odds, indikatorernas topp-4-status, spår, klass, bana, kusk, styrka och startfält.';
